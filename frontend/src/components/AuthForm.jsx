...
const AuthForm = ({mode, formData, onChange, onSubmit, loading, error, localError}) => {
  ...
  return (
    <div className={`min-h-screen flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8 ${isLogin ? 'bg-linear-to-br from-blue-50 via-indigo-50 to-purple-50' : 'bg-linear-to-br from-purple-50 via-pink-50 to-red-50'}`}>
        <div className='max-w-md w-full'>
            <div className='text-center mb-8'>
                <div className={`inline-flex items-center justify-center w-16 h-16 rounded-2xl shadow-lg mb-4 ${isLogin ? 'bg-linear-to-br from-blue-600 to-indigo-600' : 'bg-linear-to-br from-purple-600 to-pink-600 '}`}>
                    {isLogin ? (
                ...
                <p className='text-gray-600'>
                    {isLogin ? APP_CONFIG.APP_TAGLINE : `Start your learning journey today`}
                </p>
            </div>
            <div className='bg-white rounded-2xl shadow-xl p-8 border border-gray-100'>
                <div className='mb-6'>
                    <h2 className='text-2xl font-bold text-gray-900 text-center'>
                        {isLogin ? APP_CONFIG.AUTH_CONTENT.LOGIN.HEADING : APP_CONFIG.AUTH_CONTENT.REGISTER.HEADING}
                    </h2>
                    <p className='mt-2 text-center text-sm text-gray-600'>
                        {isLogin ? APP_CONFIG.AUTH_CONTENT.LOGIN.DESCRIPTION : APP_CONFIG.AUTH_CONTENT.REGISTER.DESCRIPTION}
                    </p>
                </div>
                        ...
                        <div>
                            ...
                            <div className='relative'>
                                ...
                                <input 
                                    ...
                                    onChange={onChange}
                                    className={`block w-full pl-10 pr-3 py-3 border border-gray-300 rounded-lg focus:ring-2 transition-colors ${isLogin ? 'focus:ring-blue-500 focus:border-blue-500' : 'focus:ring-purple-500 focus:border-purple-500'}`}
                                    placeholder={isLogin ? 'Enter your new password' : 'Minimum 6 characters'}
                                />
                            </div>
                        </div>
                        ...
                        >
                            {loading ? (
                                <>
                                    ...
                                    {isLogin ? 'Sign In...' : 'Creating account...'}
                                </>
                            ): (
