...
export const useZego = () => {
    ...
    const joinZegoRoom = useCallback(async(roomId) => {
        ...
        try {
            ...
            if(container.offsetWdith === 0 || container.offSetHeight === 0){
                console.error('Container has zero deminsions, waitng a bot more...')
            ...
            const errorMessage = error.message || 'Failed to joim room. Please check your camera.microphone permission and try again'
            setError(errorMessage)
            setIsJoined(false)
            setUserHasJoined(false)
        ...
        return () => {
            if(joinedRoomIdRef.current && !isLeavingRef.current){
                try {
                    ...
                    console.error('Error in cleaning leave room', error)
                }
                joinedRoomIdRef.current = null;
                isJoiningRef.current = false;
                isLeavingRef.current = false
